...
function showQuestion() {
    ...
    // Guardamos el texto de la respuesta correcta antes de mezclar
    const correctText = qData.a[qData.c]; 

    // Creamos una copia de las opciones y las mezclamos
    const shuffledOptions = [...qData.a].sort(() => Math.random() - 0.5);
    ...
    shuffledOptions.forEach((opt) => {
        ...
        btn.innerText = opt;
        // Ahora comparamos por texto, no por índice fijo
        btn.onclick = () => selectAnswer(opt, correctText, btn);
        container.appendChild(btn);
    });
}

function selectAnswer(selectedText, correctText, btnElement) {
    ...
    if (selectedText === correctText) {
        ...
        // Buscamos el botón que tiene el texto correcto para resaltarlo
        buttons.forEach(b => {
